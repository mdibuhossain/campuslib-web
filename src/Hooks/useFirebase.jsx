...
const useFirebase = () => {
  ...
  const [changePhoto, { loading: updateProfileLoading }] =
  ...
  const {
    data: { isAdmin: { isAdmin: admin = false } = {} } = [],
    loading: adminLoading = true,
  } = useQuery(GET_ADMIN, {
  ...
  useEffect(() => {
    const unsubscribed = onAuthStateChanged(auth, (user) => {
      if (user) {
        setUser(user);
        getIdToken(user).then((idToken) => setToken(`Bearer ${idToken}`));
        clearUser();
